...
- [设计原理](#设计原理)
- [它如何工作](#它如何工作)
- [Ombre 兼容性说明](#ombre-兼容性说明)
- [Quick Start](#quick-start)
  - [路径 0：原地扩展（推荐）](#路径-0原地扩展现有-ombre-部署推荐)
- [配置说明](#配置说明)
...
Night-Fall 的 auto-surface 机制（梦随 breath 自动浮现）依赖对 Ombre `server.py` 的一处 patch。原版 Ombre 的 `breath` 在**无 query、仅传 affect（valence/arousal）**时会走"无参浮现分支"，该分支在返回前从未调用 Night-Fall 的 auto-surface hook，导致梦静默失效。

Night-Fall 修复了这个问题：在无参浮现分支的末尾加入 auto-surface 检查，条件是 `valence != -1 or arousal != -1`。
...
| 我的 Ombre 怎么跑的 | 对应路径 |
|---|---|
| 已经有 Ombre 部署（云端或本地），想原地加 Night-Fall | [路径 0：原地扩展（推荐）](#路径-0原地扩展现有-ombre-部署推荐) |
...
| 部署在 Zeabur（全新部署） | [路径 C：云端 Zeabur](#路径-c云端-zeabur) |
| 部署在 Render（全新部署） | [路径 D：云端 Render](#路径-d云端-render) |

> **强烈建议：** 如果你已经有一个跑着的 Ombre（尤其是云端部署、卷里已有数据），优先走**路径 0**。路径 C/D 是"新建一个 Night-Fall 服务替换旧 Ombre"，要处理卷迁移、URL 切换、Claude 配置重连——一不小心就静默丢数据或卡 401。路径 0 是改两行 Ombre 自己的 Dockerfile，原地 rebuild，**数据/URL/Claude 配置全不动**。

### 路径 0：原地扩展现有 Ombre 部署（推荐）

适用于已经有一个跑着的 Ombre 部署（云端 Zeabur/Render，或本地 docker），希望**零迁移**接入 Night-Fall 的用户。

思路：不新建任何服务，直接改 Ombre 自己的 `Dockerfile`，让它在构建时 pip install Night-Fall，再把启动命令换成 Night-Fall 的 launcher。Ombre 服务原地 rebuild 之后就同时是 Night-Fall。

**1. 在你的 Ombre fork 里改 Dockerfile**

打开你的 Ombre 仓库的 `Dockerfile`（如果用上游 P0luz/Ombre-Brain，先 fork 一份到自己账号），在 `EXPOSE 8000` 之后、原 `CMD` 之前插入：
...
然后把原来的 `CMD ["python", "server.py"]` 改成：

```dockerfile
CMD ["python", "-m", "night_fall.launcher"]
```

**2. 应用 server.py patch**

打开同一个 fork 的 `server.py`，在无参浮现分支末尾（`return "\n\n".join(parts)` 之前）插入：
...
（没有这一步，`breath(arousal=X)` 不会触发自动浮梦。详见 [Ombre 兼容性说明](#ombre-兼容性说明)。）

**3. push 到你的 Ombre fork，原地 rebuild**

```bash
git add Dockerfile server.py
git commit -m "feat: integrate Night-Fall extension"
git push
```

- **Zeabur/Render**：检测到 push 后自动 rebuild，URL/卷/环境变量全部沿用，部署完成后 Claude Desktop 不需要改任何配置
- **本地 docker**：`docker compose up --build -d`

**4. 验证**

Claude Desktop 重连之后，应该能看到原来的 Ombre 工具加上 `night_fall`。然后：

```
night_fall(action="generate")    # 攒一个潜伏梦
# 等 3 小时
breath(arousal=0.6)              # 看是否浮现 === 浮上来的梦 === 区块
```

**为什么这条路最稳？**

- 卷不动 → 旧记忆桶原地继续用
- URL 不动 → Claude Desktop 配置不动
- 环境变量不动 → `OMBRE_API_KEY` 等照旧
- 出问题直接 revert 那个 commit，rebuild，回到纯 Ombre 状态，**数据完全无损**
...
潜伏期过后（默认 3 小时），下一次带 affect 的 `breath` 会自动尝试浮现——无需额外调用任何工具：

```
breath(arousal=0.6)                    # 有 affect 即可触发
breath(query="今天很累", valence=0.3)   # query + affect 也行
```

浮上来的梦以 `=== 浮上来的梦 ===` 为前缀追加在 breath 返回的末尾，交付一次后物理删除。要保留，在当轮显式 `hold()`。

> `night_fall(action="surface")` 仍然保留，作为手动调试入口，正常流程无需调用。
